...
st.markdown('说明(1):此处的装机总量约束用于某一技术在所有地点的总规模约束,比如本项目只有100MW光伏指标,即光需要将光伏的最大装机容量设为100MW.')
st.markdown('说明(2):此处的需求总量区分时间和地点. 其中,对于小时固定需求,需要输入8760小时的需求曲线; 对于日内可调节需求, 需要输入365天每日总需求, 和每日每小时的最大需求和最小需求.')
st.markdown('说明(3):若一个地点有多个用户, 需要外生将其归为小时固定类或日内可调节类, 分别加总后输入.')
st.markdown('说明(4):输入数据的模板行、列、页名称均不可修改, 只能修改需求数字. 氢、氨需求的单位均为吨, 电需求的单位均为MWh.')
...
create_dem_template(node_available_tech_constraint)

with st.container():
    st.markdown('### 输入电氢氨醇需求数据')
    st.markdown('先下载数据模板, 填写后再上传即可。')
    with open("./.cache/DemandDataTemplate.xlsx", "rb") as file:
        btn = st.download_button(
            label="下载各个地点电、氢、氨、甲醇需求数据的输入模板",
            data=file,
            file_name="电氢氨醇需求数据模版.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key='down_dem_template'
        )

    uploaded_dem_file = st.file_uploader(
        "请上传填写好的模板", accept_multiple_files=False,key='up_dem_template'
    )

    if uploaded_dem_file is not None:
        with pd.ExcelFile(uploaded_dem_file) as w:
            power_dem_hourfix=pd.read_excel(w,sheet_name='小时固定的电力需求',index_col=0)
            h2_dem_hourfix=pd.read_excel(w,sheet_name='小时固定的氢需求',index_col=0)
            nh3_dem_hourfix=pd.read_excel(w,sheet_name='小时固定的氨需求',index_col=0)
            ch3oh_dem_hourfix=pd.read_excel(w,sheet_name='小时固定的甲醇需求',index_col=0)

            power_dem_shiftable=pd.read_excel(w,sheet_name='日内可调节的电力需求',index_col=0)
            h2_dem_shiftable=pd.read_excel(w,sheet_name='日内可调节的氢需求',index_col=0)
            nh3_dem_shiftable=pd.read_excel(w,sheet_name='日内可调节的氨需求',index_col=0)
            ch3oh_dem_shiftable=pd.read_excel(w,sheet_name='日内可调节的甲醇需求',index_col=0)

            shiftable_dem_info=pd.read_excel(w,sheet_name='日内可调节需求的参数',index_col=0)

        st.markdown('请自行对照模板,检查上传文件的表头等是否有错误')
        check_upload_file=st.columns([1,1,1,1,1,1,1,1,1])
        check_upload_file[0].markdown('小时固定的电力需求')
        check_upload_file[0].dataframe(power_dem_hourfix,height=5)
        check_upload_file[1].markdown('小时固定的氢需求')
        check_upload_file[1].dataframe(h2_dem_hourfix,height=5)
        check_upload_file[2].markdown('小时固定的氨需求')
        check_upload_file[2].dataframe(nh3_dem_hourfix,height=5)
        check_upload_file[3].markdown('小时固定的甲醇需求')
        check_upload_file[3].dataframe(ch3oh_dem_hourfix,height=5)
        check_upload_file[4].markdown('日内可调节的电力需求')
        check_upload_file[4].dataframe(power_dem_shiftable,height=5)
        check_upload_file[5].markdown('日内可调节的氢需求')
        check_upload_file[5].dataframe(h2_dem_shiftable,height=5)
        check_upload_file[6].markdown('日内可调节的氨需求')
        check_upload_file[6].dataframe(nh3_dem_shiftable,height=5)
        check_upload_file[7].markdown('日内可调节的甲醇需求')
        check_upload_file[7].dataframe(ch3oh_dem_shiftable,height=5)
        check_upload_file[8].markdown('日内可调节需求的参数')
        check_upload_file[8].dataframe(shiftable_dem_info,height=5)
...
model_name=st.text_input("输入模型名称:", "default001")
...
try:
    input_coef={
        ...
        'power_dem_hourfix':power_dem_hourfix.to_dict(),
        'h2_dem_hourfix':h2_dem_hourfix.to_dict(),
        'nh3_dem_hourfix':nh3_dem_hourfix.to_dict(),
        'ch3oh_dem_hourfix':ch3oh_dem_hourfix.to_dict(),
        'power_dem_shiftable':power_dem_shiftable.to_dict(),
        'h2_dem_shiftable':h2_dem_shiftable.to_dict(),
        'nh3_dem_shiftable':nh3_dem_shiftable.to_dict(),
        'ch3oh_dem_shiftable':ch3oh_dem_shiftable.to_dict(),
        'shiftable_dem_info':shiftable_dem_info.to_dict()}
